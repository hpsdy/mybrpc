...
static void cleanup_pthread(void* arg) {
    KeyTable* kt = static_cast<KeyTable*>(arg);
    {
        ...
        std::cout << "cleanup_pthread:" << bthread_self() << "(" << (void*)kt << ")" << std::endl;
    }
    if (kt) {
        delete kt;
...
size_t get_keytable_count1(void*) {
    return nkeytable.load(butil::memory_order_relaxed);
}
...
int bthread_setspecific(bthread_key_t key, void* data) {
	...
	{
            BAIDU_SCOPED_LOCK(bthread::s_key_mutex);
            std::cout << "tls_ever_created_keytable:" << bthread_self() << "(" << bthread::tls_ever_created_keytable << ")" << "; KeyTable:" << (void*)kt << std::endl;
        }
    ...
    {
        BAIDU_SCOPED_LOCK(bthread::s_key_mutex);
        std::cout << "KeyTable:" << (void*)kt << std::endl;
    }
