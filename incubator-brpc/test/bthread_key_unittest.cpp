...
void dtor2(void* arg) {
    ...
    std::cout << "finish:" << (size_t)arg << std::endl;
}
static pthread_mutex_t test_key_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static void worker2_impl(bthread_key_t k) {
    ...
    {
        BAIDU_SCOPED_LOCK(test_key_mutex);
        std::cout << "key num:" << bthread::get_key_count1(NULL) << "; get_keytable_count:" << bthread::get_keytable_count1(NULL) <<  "; data:" << (size_t)(bthread_getspecific(k)) << std::endl;
    }
    sleep(1);
}
...
TEST(KeyTest, use_one_key_in_different_threads) {
    ...
    std::vector<bthread_keytable_pool_t*> array;
    bthread_t bth[1];
    for (size_t i = 0; i < arraysize(bth); ++i) {
        bthread_keytable_pool_t *pool = new bthread_keytable_pool_t;
        pool->free_keytables = NULL;
	pool->destroyed = 0;
        pthread_mutex_init(&pool->mutex, NULL);
        array.push_back(pool);
        bthread_attr_t *args = new bthread_attr_t{BTHREAD_STACKTYPE_NORMAL, 0, pool};	
        ASSERT_EQ(0, bthread_start_urgent(&bth[i], args, worker2, &k));
    }
    for (size_t i = 0; i < arraysize(bth); ++i) {
        ASSERT_EQ(0, bthread_join(bth[i], NULL));
    }
